...
static char *memfd_name(int fd, const char *path, char *name, size_t size)
{
    int r = snprintf(name, size-1, "memfd://%s", path);
    if (r <= 0 || r >= (ssize_t)size-1)
        error("failed to generate memfd name for (%d): %s", fd,
            strerror(errno));
    return name;
}
