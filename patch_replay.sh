...
for mdir in $outfd/queue/*; do
  ...
  if [ -d "$mdir" ]; then
    ...
    for patch_file in "$mdir"/*.patch; do
      ...
      # rm -rf "${patch_file%.patch}.log"
      echo "Executing: ./replay_and_log "${patch_file%.patch}.log" $patch_file '-o $outfd'"
      ./replay_and_log.sh "${patch_file%.patch}.log" $patch_file \"-o ptoftpd-out/proftpd-1\"

      # cd $expdir
      
      # if [[ i -eq 1 && $outfd = "proftpd-out/proftpd-1" ]]; then
      ...
      # echo "Copying proftpd-gcov to proftpd_copy_$i"
      ...
      echo "complete '${patch_file%.patch}.log'"
